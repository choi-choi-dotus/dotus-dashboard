...
with st.sidebar:
    ...
    # 카테고리 필터
    st.markdown("### 📦 상품 대분류")
    all_cats = sorted(df_sales["large_category"].dropna().unique())
...
filtered = df_sales[
    ...
    (df_sales["large_category"].isin(selected_cats))
]
...
st.markdown(f"**조회 기간:** {month_start} ~ {month_end}　|　**채널:** {len(selected_clients)}개　|　**카테고리:** {len(selected_cats)}개")
st.markdown("---")
